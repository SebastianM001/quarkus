...
You can override this behavior by setting `mp.messaging.incoming.$channel.broadcast=true` on a channel.
...
[NOTE]
====
Reciprocally, multiple producers on the same channel can be merged by setting `mp.messaging.outgoing.$channel.merge=true`.
